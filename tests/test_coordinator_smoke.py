...
async def server(aiohttp_server):
    app = web.Application()
    async def authorize(request): return web.Response(text='<a href="?tx=StateProperties=abc"></a>')
    async def self_asserted(request): return web.Response(text="{}")
    async def confirm(request): return web.Response(text="{}")
    async def authorize_silent(request): raise web.HTTPFound(location="msauth.com.mazdausa.mazdaiphone://auth?code=AUTH_CODE")
    async def token(request): return web.json_response({"access_token":"t1","refresh_token":"r1","expires_in":3600})
    async def vehicles(request): return web.json_response([{"vin":"JMZTEST","id":"1","modelName":"MX-30"}])
    async def status(request): return web.json_response({"batteryPercentage":80})
